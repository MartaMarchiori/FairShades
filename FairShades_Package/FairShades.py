...
class FairShades(object): 
  def toExplain(self, dataset, subset_records, Wrapper): 
    self.dataset = dataset
    self.subset_records = subset_records 
    self.Wrapper = Wrapper
    real_labels=dataset[:subset_records][1]
    w1 = Wrapper()
    pred_proba=w1.predict(dataset[:subset_records][0])
  ...
  def neighbourhoodPrediction(self,index_sentence_to_explain,sentence_to_explain,building_predicted_for_neigh):
    ...
    self.building_predicted_for_neigh=building_predicted_for_neigh
    self.correct = True
    ...
    predictions = building_predicted_for_neigh(self.neigh)
  ...
  def explain(self, explanationType, corpus=None):
    if explanationType == 'local':
      ...
      l_expl=l_x.generate_l_explanation(self.in_input, self.neigh, self.sentence_to_explain, self.real_label_sentence_to_explain, self.correct)
      return l_x
    elif explanationType == 'global':
      inputs=build_global_inputs(corpus,self.samples,self.building_predicted_for_neigh)
